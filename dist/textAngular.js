...
angular.module('textAngular.DOM', ['textAngular.factories'])
    .factory('taExecCommand', ['taSelection', 'taBrowserTag', '$document', function (taSelection, taBrowserTag, $document) {
        ...
        function ($document, taDOM, $log) {
            ...
            var bShiftState;
            var brException = function (container, element, offset) {
                ...
                */
                if (element.tagName && element.tagName.match(/^br$/i) && offset === 0 && !element.previousSibling) {
                    ...
                    if (container.childNodes) {
                        var i = 0, l = container.childNodes.length;

                        for (i; i < l; i++) {
                            var elm = element.nodeType === 3 ? element.parentNode : element;
                            var childNode = container.childNodes[i];
                            if (childNode.isEqualNode(elm)) {
                                return {
                                    element: element,
                                    offset: i
                                };
            ...
            var api = {
                getSelection: function () {
                    ...
                    var selection = {
                        start: brException(container, range.startContainer, range.startOffset),
                        end: brException(container, range.endContainer, range.endOffset),
                        ...
                        if (!_isContentEditable) {
                            ...
                            var processpaste = function (text) {
                                ...
                                if (text && text.trim().length) {
                                    ...
                                    text = text.replace(/href=".*?(?=#\/wcm\/)/gi, 'href="');
